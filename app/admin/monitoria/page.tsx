...
// ✅ Interface ajustada para os dados que a API /api/monitorias-agrupadas retorna
interface MonitoriaAgrupadaDisplay {
  id_representativo: number;
  nome_disciplina: string;
  nome_monitor: string;
  nomes_alunos: string; // <- Recebido como uma string única
  conteudo_programatico: string;
  data_agendamento: string;
  status: string;
}

// Interface para carregar dados de uma sessão para edição
...
  const [monitoriasAgrupadas, setMonitoriasAgrupadas] = useState<MonitoriaAgrupadaDisplay[]>([]);
...
    try {
      // ✅ Chamando a rota correta que retorna os dados agrupados
      const response = await axios.get<MonitoriaAgrupadaDisplay[]>('/api/monitorias-agrupadas');
...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <div className="max-w-screen-xl mx-auto p-4 md:p-6 mt-16 md:mt-20">
        ...
        {/* Container da Grade (Grid) */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          
          {/* Coluna da Esquerda (Formulário - 2/3 da largura em telas grandes) */}
          <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 md:p-8">
            ...
            {errorForm && (
              <div className="bg-red-100 border-l-4 border-red-500 text-red-700 dark:text-red-200 dark:bg-red-900/50 p-4 mb-6 rounded-md shadow-md" role="alert">
                <p className="font-bold">Erro!</p>
                <p>{errorForm}</p>
              </div>
            )}
            {successMessage && (
              <div className="bg-green-100 border-l-4 border-green-500 text-green-700 dark:text-green-200 dark:bg-green-900/50 p-4 mb-6 rounded-md shadow-md" role="alert">
          ...
          {/* Coluna da Direita (Lista de Sessões - 1/3 da largura em telas grandes) */}
            ...
            <div className="max-h-[600px] overflow-y-auto space-y-4 pr-2 custom-scrollbar">
              {loadingMonitorias ? (
                ...
                monitoriasAgrupadas.map((sessao) => (
                  <div key={sessao.id_representativo} className="bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow">
                    <h4 className="text-md font-semibold text-blue-600 dark:text-blue-400">{sessao.nome_disciplina}</h4>
                    <p className="text-xs text-gray-500 dark:text-gray-400">ID Grupo Rep.: {sessao.id_representativo}</p>
                    ...
                    <p className="text-sm text-gray-700 dark:text-gray-300">
                      <strong>Alunos:</strong> {sessao.nomes_alunos || 'Nenhum aluno'}
                    </p>
                    <p className="text-sm text-gray-700 dark:text-gray-300">
                    ...
                    <div className="mt-3 flex gap-2 justify-end">
                      <button
                        onClick={() => handleEditClick(sessao.id_representativo)}
                      ...
                      <button
                        onClick={() => handleDeleteClick(sessao.id_representativo)}

...
import CalendarioAgendamento from '@/app/components/calendarioAgendamento'; // Importa o componente de calendário

export default function AdminDashboard() { // Renomeado para AdminDashboard para clareza
  const userType: 'admin' | 'monitor' | 'student' = 'admin'; // Defina corretamente o tipo de usuário como 'admin'
  const chartRef = useRef<HTMLCanvasElement | null>(null);
  const chartInstanceRef = useRef<Chart | null>(null);

  // Estados para armazenar os dados do gráfico e o nome do usuário
  const [chartLabels, setChartLabels] = useState<string[]>([]);
  const [chartMedias, setChartMedias] = useState<number[]>([]); // Estado para as médias das notas
  const [userName, setUserName] = useState<string>(''); // Estado para o nome do administrador
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Faz a requisição para a API do administrador
        ...
        console.log("Dados recebidos da API do Administrador:", { fetchedUserName, fetchedData });
        ...
        const medias = fetchedData.map((item: any) => parseFloat(item.media_nota)); // Converte para número

        setChartLabels(labels);
        setChartMedias(medias); // Atualiza o estado das médias
        setUserName(fetchedUserName); // Define o nome do administrador

      } catch (err: any) {
        console.error("Erro ao carregar dados do dashboard do administrador:", err.message);
  ...
  // useEffect para criar/atualizar o gráfico quando os dados (chartLabels, chartMedias) mudam
  useEffect(() => {
    // Só cria o gráfico se houver dados e o canvas estiver pronto
    if (chartRef.current && chartLabels.length > 0) {
      ...
      if (ctx) {
        ...
        // Mapeia as médias para as cores, ciclando pelas cores disponíveis
        const backgroundColors = chartMedias.map((_: any, index: number) => colors[index % colors.length]);
        ...
        chartInstanceRef.current = new Chart(ctx, {
          ...
          data: {
            ...
            datasets: [
              {
                label: "Média da Nota", // Rótulo do conjunto de dados
                data: chartMedias, // Valores do estado
          ...
          options: {
            ...
            plugins: {
              ...
              title: { // Título principal do gráfico
                display: true,
                text: 'Média de Desempenho por Disciplina (Geral)', // Título para o desempenho geral
                color: '#333',
                font: {
                  size: 16,
                  weight: 'bold'
  ...
  }, [chartLabels, chartMedias, loading, error]); // Dependências do useEffect

  // Renderização condicional baseada no estado de carregamento e erro
  if (loading) {
    return <div className="flex justify-center items-center h-screen text-lg">Carregando dashboard do administrador...</div>;
  }
    ...
    <div className="flex">
      ...
      <div className="container mx-auto px-4 py-6 flex-1">
        ...
        <div className="flex-1 p-10">
          {/* Título da página com nome do administrador */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              <div className="flex justify-center items-center bg-gray-100 border h-52 text-gray-500 text-lg">
                <CalendarioAgendamento/> {/* Componente de calendário */}
              </div>
            </div>

            {/* Card do gráfico de desempenho */}
            <div className="bg-white p-6 rounded-lg shadow-md">
              <h5 className="text-lg font-semibold mb-4">Desempenho Geral dos Alunos</h5> {/* Título do card */}
              <div className="h-52">
                {/* Renderiza o gráfico se houver dados, senão mostra uma mensagem */}
                {chartLabels.length > 0 ? (
                  <canvas ref={chartRef}></canvas>
                ) : (
                  <p className="text-center text-gray-500 pt-10">Nenhum dado de desempenho encontrado para exibir.</p>
                )}
              </div>
            </div>
          </div>
